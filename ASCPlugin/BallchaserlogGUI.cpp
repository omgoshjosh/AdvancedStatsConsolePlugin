...
std::string ASCPlugin::GetMenuName()
{
	return "ASCPlugin";
}

// Title to give the menu
std::string ASCPlugin::GetMenuTitle()
{
	return menuTitle_;
}

// Don't call this yourself, BM will call this function with a pointer to the current ImGui context
void ASCPlugin::SetImGuiContext(uintptr_t ctx)
{
	ImGui::SetCurrentContext(reinterpret_cast<ImGuiContext*>(ctx));
}

// Should events such as mouse clicks/key inputs be blocked so they won't reach the game
bool ASCPlugin::ShouldBlockInput()
{
	return ImGui::GetIO().WantCaptureMouse || ImGui::GetIO().WantCaptureKeyboard;
}

// Return true if window should be interactive
bool ASCPlugin::IsActiveOverlay()
{
	return true;
}

// Called when window is opened
void ASCPlugin::OnOpen()
{
	isWindowOpen_ = true;
	api->GetLastMatches();
}
...
void ASCPlugin::RenderReplayDetail(GetReplayResponseData* detail)
{
...
void ASCPlugin::ContextMenu(std::vector<TableColumn>& columnData)
{
...
void ASCPlugin::RenderTableTab(std::string name, TableSettings& settings, GetReplayResponseData* detail, bool bBlueHeader, bool bOrangeHeader)
{
...
void ASCPlugin::RenderTeamTable(Team& team, TableSettings& settings, bool drawHeader)
{
